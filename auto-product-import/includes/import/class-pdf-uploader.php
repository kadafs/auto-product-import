...
class APM_PDF_Uploader {
    ...
    public function upload($pdf_url, $caption, $product_id, $debug = false) {
        ...
        if ($existing_attachment) {
            ...
            // Still add to Downloads category if not already
            $this->add_to_downloads_category($existing_attachment, $debug);
    ...
    /**
     * Add PDF to Downloads or Manuals media category
     * Fixed to work with Media Library Organizer plugin
     */
    private function add_to_downloads_category($attachment_id, $debug = false) {
        // Media Library Organizer uses 'mlo-category' taxonomy
        $taxonomies = array('mlo-category', 'media_category', 'mla_media_category', 'ml-category');
        ...
        foreach ($taxonomies as $taxonomy) {
            if (taxonomy_exists($taxonomy)) {
                if ($debug) {
                    error_log("APM: Found media taxonomy: $taxonomy");
                }
                
                // Search for Downloads or Manuals category (case-insensitive)
                $search_terms = array('Downloads', 'downloads', 'Manuals', 'manuals');
                $term = null;
                
                foreach ($search_terms as $search_term) {
                    $term = get_term_by('name', $search_term, $taxonomy);
                    if ($term) {
                        if ($debug) {
                            error_log("APM: Found existing category: $search_term (ID: {$term->term_id})");
                        }
                        break;
                    }
                }
                
                // If not found, try to create "Downloads" category
                if (!$term) {
                    $term_data = wp_insert_term('Downloads', $taxonomy, array(
                        'slug' => 'downloads'
                    ));
                    ...
                    if (!is_wp_error($term_data)) {
                        ...
                        if ($debug) {
                            error_log("APM: ✓ Created 'Downloads' category in taxonomy: $taxonomy (ID: $term_id)");
                        }
                    } else {
                        if ($debug) {
                            error_log("APM: ⚠ Could not create 'Downloads' category: " . $term_data->get_error_message());
                        }
                        continue; // Try next taxonomy
                    }
                } else {
                ...
                if (isset($term_id)) {
                    $result = wp_set_object_terms($attachment_id, $term_id, $taxonomy, true);
                    
                    if (!is_wp_error($result)) {
                        if ($debug) {
                            error_log("APM: ✓ Added PDF to category (taxonomy: $taxonomy, term_id: $term_id)");
                        }
                        $category_found = true;
                        return; // Success, exit
                    } else {
                        if ($debug) {
                            error_log("APM: ⚠ Failed to assign category: " . $result->get_error_message());
                        }
                    }
                }
            }
        ...
        if (!$category_found && $debug) {
            error_log("APM: ⚠ No media category taxonomy found - PDF not categorized. Install Media Library Organizer or similar plugin.");
        }
    }
